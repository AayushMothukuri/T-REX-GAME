    ...
    handleBarCodeScanned = async({type, data})=>{
     ...
     initiatebookissue=async()=>{
       db.collection("transaction").add({
         'studentId':this.state.scannedStudentId,'bookId':this.state.scannedBookId,'transactiontype':"issue"
       })
       db.collection("book").doc(this.state.scannedBookId).update({
         'bookav':false
       })
       db.collection("student").doc(this.state.scannedStudentId).update({
         'no. of book issued':firebase.firestore.FieldValue.increment(1)
       })
     ...
     initiatebookreturn=async()=>{
      db.collection("transaction").add({
        'studentId':this.state.scannedStudentId,'bookId':this.state.scannedBookId,'transactiontype':"return"
      })
      db.collection("book").doc(this.state.scannedBookId).update({
        'bookav':true
      })
